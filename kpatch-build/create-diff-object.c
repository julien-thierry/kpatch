...
static void kpatch_add_child_symbol(struct symbol *parent, struct symbol *child)
{
	...
	if (!parent->children)
		ERROR("Failed to add a child function");
...
static void kpatch_detect_child_functions(struct kpatch_elf *kelf)
{
	...
	list_for_each_entry(sym, &kelf->symbols, list) {
		...
		if (!parent) {
			/* Bundled ".part." symbols don't always have parents */
			if (sym->sec->sym == sym)
				continue;
...
static bool kpatch_child_status_has_changed(struct symbol *sym)
{
	int i;

	for (i = 0; i < sym->nb_children; i++) {
		if (sym->children[i]->sec->status == CHANGED ||
		    kpatch_child_status_has_changed(sym->children[i]))
			return true;
	}
	...
	list_for_each_entry(sec, seclist, list) {
		if (is_rela_section(sec)) {
			...
			if (sym && sym->status == SAME &&
			    kpatch_child_status_has_changed(sym))
				sym->status = CHANGED;
		}
	}
}
							  ...
							  struct symbol *sym)
{
	...
	for (i = 0; i < parent->nb_children; i++) {
		struct symbol *child = parent->children[i];
						    ...
						    struct kpatch_elf *patched)
{
	...
	list_for_each_entry(sec, &base->sections, list) {
		...
		list_for_each_entry(rela, &sec->relas, list) {
			...
			if (!kpatch_find_static_twin_ref(sec->twin, sym))
